"""
Step 5.5: Corrected trial-aligned evaluation using the CONFIRMED dataset
interval (2, 6) instead of the wrongly-assumed (0, 4).
Tracks both full-window accuracy and near-end (tightest alignment) accuracy.
"""
import argparse
import threading
import time
...
# CONFIRMED from dataset.interval
...
def main():
    ...
    try:
        ...
        print(f"Full window [{INTERVAL_START_SEC}-{INTERVAL_END_SEC}s]: {n_full_correct}/{n_full} "
              f"= {n_full_correct/n_full:.3f}" if n_full > 0 else "Full window: no samples")
        print(f"Near-end window (tightest alignment, last {NEAR_END_WINDOW_SEC}s): "
              f"{n_near_end_correct}/{n_near_end} = {n_near_end_correct/n_near_end:.3f}" if n_near_end > 0 else "Near-end: no samples")
